...
weights[ 'D_h1' ] = tf.get_variable( "D_h1",
                            [ net_metadata['input_size'], net_metadata['D_layer1_size'] ],
                            tf.float32,
                            tf.contrib.layers.xavier_initializer() )
weights[ 'D_h2' ] = tf.get_variable( "D_h2",
                            [ net_metadata['D_layer1_size'], net_metadata['out_layer_size'] ],
                            tf.float32,
                            tf.contrib.layers.xavier_initializer() )
...
# D_net has to be a func so it can connect two input; also don't do sigmoid here, return raw value
...
weights[ 'G_h1' ] = tf.get_variable( "G_h1",
                            [ net_metadata['z_size'], net_metadata['G_layer1_size'] ],
                            tf.float32,
                            tf.contrib.layers.xavier_initializer() )
weights[ 'G_h2' ] = tf.get_variable( "G_h2",
                            [ net_metadata['G_layer1_size'], net_metadata['input_size'] ],
                            tf.float32,
                            tf.contrib.layers.xavier_initializer() )
...
# feed Z into G net
G_L1 = net_metadata[ 'activate func' ]( tf.add( tf.matmul( z, weights['G_h1'] ), weights['G_b1'] ) )
G_out = net_metadata[ 'output func' ]( tf.add( tf.matmul( G_L1, weights['G_h2'] ), weights['G_b2'] ) )
...
# define cost
def DNetCcost( x ):
    return tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=x, labels=tf.ones_like(D_logit_real(x))))

# raw version
dcost = -tf.reduce_mean(tf.log(D_real) + tf.log(1. - D_fromZ)) 
gcost = -tf.reduce_mean(tf.log(D_fromZ)) 

# Alternative cost
#cost_real = DNetCost( D_raw_real )
#cost_fromZ = DNetCost( D_raw_fromZ )
...
d_optimizer = tf.train.AdamOptimizer( net_metadata['d_learning_rate'] ).minimize(dcost, var_list=dWeights)
g_optimizer = tf.train.AdamOptimizer( net_metadata['g_learning_rate'] ).minimize(gcost, var_list=gWeights)
...
for epoch in range(net_metadata['training_epoch']):
    ...
    for i in range(total_batch):
        ...
        batch_z1 = np.random.uniform(-1., 1., size=[bsz, net_metadata['z_size']])
        batch_z2 = np.random.uniform(-1., 1., size=[bsz, net_metadata['z_size']])
        
        _, dcost0 = sess.run([d_optimizer, dcost], feed_dict={x: batch_x, z: batch_z1})
        _, gcost0 = sess.run([g_optimizer, gcost], feed_dict={z: batch_z2})   
